<script lang="ts">
    ...
    function placeRandom(width, height, widthEdge, heightEdge, items) {
        ...
        console.log(width)
        componentSize = {width: mapStagingWidth / 4, height: mapStagingHeight / 8};
        let placedItems = [];
        let loops = 0;
        for (var i = 0; i < items.length; i++) {
            ...
            do {
                loops++;
                top = Math.floor(Math.random() * (mapStagingHeight - height) + heightEdge);
                left = Math.floor(Math.random() * (mapStagingWidth - width) + widthEdge);
            } while (
                (placedItems.some(item => isOverlapping({top, left}, item, width,height))) && loops < 1000);
    ...
    async function updateData(repository) {
        try {
            ...
            repositoryComponents = placeRandom(componentSize.width, componentSize.height, window.innerWidth / 6, window.innerHeight * 0.07, await repositoryComponentsResponse.json());
        } catch (error) {
            console.error(error);
        }
    }
...
<div class="flex flex-grow p-2 h-full justify-end">
            ...
            <SidebarGroup class="pb-2 h-32">
                ...
                <div class="flex space-x-2 h-1/2 py-2">
                    <div class="w-2/3 h-full">
                        <Input bind:value={repoSearchInput} oninput={() => updateSearch()} class="h-full"></Input>
                    </div>
                    <div class="w-1/3 h-full flex">
                        <Button>Filter By<ChevronDownOutline class=" h-full text-white dark:text-white" /></Button>
                        <Dropdown bind:isOpen={filterOpen} simple>
    ...
    <div bind:this={mapStagingSize} class="flex flex-col gap-2 w-7/12">
        {#each repositoryComponents ?? [] as component}
            <MetricCard title={component.name} health={component.health} left={component.left} top={component.top} onResize={({width, height}) => {
